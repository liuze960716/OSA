...
Slider.prototype.createUI = function(){
	...
	//ul li	
	htmlstr='<ul style="position:absolute;list-style:none;right:663px;bottom:0px;z-index:1;">';
	for(let i=0;i<this.imgs.length;i++){
		htmlstr+= '<li style="margin-right:20px;float:left;';
		htmlstr+='width:'+this.btnSize+"px;";
		htmlstr+='height:'+this.btnSize+"px;";
		if(this.isCircle){
			htmlstr+='border-radius:80px;';
		}
...
Slider.prototype.showImg=function(outIndex,inIndex){
 ...
 //            })
 //            //对两张图片进行淡入淡出
 //            $("outIndex").eq(num).fadeOut("fast","swing");
 //            $("inIndex").eq(count).fadeIn("fast","swing");
 //        },3000)
